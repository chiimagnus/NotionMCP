...
test("read_file 只读 UTF-8 普通文件，并校验范围、大小、二进制与取消", async (t) => {
	...
	const searchSkills = await import(`../tools/search_skills.mjs?test=${Date.now()}-${Math.random()}`)
	...
	const discovered = await searchSkills.call({ query: "development", limit: 1 })
	assert.match(discovered.content[0].text, /skills\/alpha/)
	assert.doesNotMatch(discovered.content[0].text, /ALPHA_BODY_MUST_NOT_BE_DISCOVERED/)
	assert.match((await loadSkills.call({ name: "skills/alpha" })).content[0].text, /ALPHA_BODY_MUST_NOT_BE_DISCOVERED/)
	assert.match((await searchSkills.call({ query: "nested review" })).content[0].text, /skills\/nested\/beta/)
	assert.equal((await searchSkills.call({ query: 1 })).isError, true)
	assert.equal((await searchSkills.call({ query: "alpha", limit: 0 })).isError, true)
	...
	assert.match((await searchSkills.call({ query: "refreshed" })).content[0].text, /skills\/gamma/)
})
