...
test("read_file 只读 UTF-8 普通文件，并校验范围、大小、二进制与取消", async (t) => {
	...
	const formattedRules = agents.formatAgentsMdContext(hierarchy)
	assert.doesNotMatch(formattedRules, /\[read rules\]|digest:|workdir:/)
	assert.match(formattedRules, /--- AGENTS\.md: .*global[\\/]AGENTS\.md ---/)
	...
	assert.match((await readRules.call({ cwd: nested })).content[0].text, /path: .*skills\/gamma\/SKILL\.md\n  name: Gamma\n  description: refreshed catalog/)
})
