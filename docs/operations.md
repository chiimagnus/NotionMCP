...
不要用 `tailscale funnel reset` 作为排障手段：它会影响非本项目 route。NotionMCP 只配置和卸载自己的三条 MCP 路径：`/mcp`、`/mcp/sse` 与 `/mcp/messages`。
...
`activeRequests` 是正在执行的 MCP 请求，`queuedRequests` 是 FIFO 等待数；它们分别不能超过对应的 `max...` 字段。`connections.streamableSse` 是 Notion 仍会使用的 2025 GET 通知流，`connections.legacySse` 是显式 2024 HTTP+SSE 会话；`totalSse` 与 `sseHighWater` 用于判断连接是否累积。达到 `maxSseStreams` 或 `maxQueuedRequests` 时，只会拒绝**新**连接/请求，并返回 `Retry-After: 1`；已建立 SSE 不会被服务主动踢掉。
...
- `sse_opened` / `sse_closed` 配合 `streamableSseSessions` 或 `legacySseSessions` 用于确认连接是否成对回收；日志只记录本地序号，不记录 SSE session ID、Token、请求 body、命令或文件内容。

## 两轮 12 条真实 Notion 复测

本地测试不能替代此验收。升级并重启常驻服务后，先确认 `npm run doctor` 的 `local`、`funnel` 都是 `ready`，并在本机另开终端执行 `tail -f mcp.log` 与上面的 `/healthz` 查询。
