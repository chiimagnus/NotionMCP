...
// stdio 的基本约定：客户端关闭 stdin = 不再需要这个进程。之前没处理这个事件，每个
// 结束的会话都留下一个约 50MB 的孤儿进程，只增不减（up.mjs 的定期回收只是周期
// 性清扫，治标不治本；上游 supergateway 不杀子进程是诱因，但 stdio 服务端本来
// 就该在 stdin EOF 时自行退出）。
//
// 这与上面 fatal() 里「stdout EPIPE 不退出」并不矛盾，恰恰是一体两面：写不出去不
// 代表没人要结果，不该死；stdin EOF 是客户端明确说完了，必须死。只修前一半，
// 孤儿进程反而会活得更久。
let exiting = false
function exitOnStdinClose(reason) {
	if (exiting) return
	exiting = true
	log(`stdin 已关闭（${reason}），客户端不再需要本进程，退出以免残留孤儿进程`)
	process.exit(0)
}
process.stdin.once("end", () => exitOnStdinClose("end"))
process.stdin.once("close", () => exitOnStdinClose("close"))
