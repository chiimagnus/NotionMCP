...
export function servicePaths(kind, home = homedir()) {
...
export async function renderPlatformService({ kind = platform(), home, nodePath = process.execPath, root = ROOT, logPath = join(ROOT, "mcp-service.log") } = {}) {
	const paths = servicePaths(kind, home)
...
export async function installPlatformService(config, { kind = platform(), home, run = runCommand, dryRun = false } = {}) {
	const service = await renderPlatformService({ kind, home })
	...
	await writeFile(service.file, service.content, kind === "windows" ? "utf16le" : "utf8")
	if (kind === "linux") {
...
export async function uninstallPlatformService(config, { kind = platform(), home, run = runCommand, dryRun = false } = {}) {
	const service = servicePaths(kind, home)
