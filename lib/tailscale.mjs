...
export function inspectFunnelStatus(result, port, now = Date.now) {
	...
	let route
	let legacyRootTarget
	for (const [host, server] of Object.entries(status.Web || {})) {
		const handlers = server?.Handlers || {}
		const allowed = status.AllowFunnel?.[host] === true
		if (handlers["/"]?.Proxy === target) legacyRootTarget = target
		if (handlers[FUNNEL_PATH]) route = { host, allowed, target: handlers[FUNNEL_PATH].Proxy }
	}
	if (!route) return { state: "missing", checkedAt, legacyRootTarget, target }
	if (route.target !== target) return { state: "wrong_target", checkedAt, legacyRootTarget, target, actualTarget: route.target }
	if (!route.allowed) return { state: "disabled", checkedAt, legacyRootTarget, target }
	return { state: "ready", checkedAt, legacyRootTarget, target, publicUrl: `https://${route.host}${FUNNEL_PATH}` }
}
...
export function createTailscaleManager({ path, run, now = Date.now, timeoutMs = 10_000 }) {
	...
	return {
		...
		async ensureMcpFunnel(port) {
			...
			if (status.state === "wrong_target") {
				throw new Error(`Funnel ${FUNNEL_PATH} 已指向其他目标：${status.actualTarget}`)
			}
			let changed = false
			if (status.legacyRootTarget === status.target) {
				await turnOff("/")
				changed = true
			}
			...
			return { changed: true, status: { ...status, state: "configured" } }
		},
