...
// 默认值可由 .env 覆盖；并发限制和 2024 legacy SSE 容量保留调校旋钮，避免真实网络条件只能改代码。
...
function stringSetValue(key) {
	const value = rawValue(key)
	if (value === undefined) return new Set()
	if (typeof value !== "string") throw new Error(`配置 ${key} 必须是字符串`)
...
export const SKILLS_IGNORE = stringSetValue(`SKILLS_IGNORE_${currentPlatform().toUpperCase()}`)
...
export const MAX_ACTIVE_REQUESTS = numberValue("MCP_MAX_ACTIVE_REQUESTS", { max: 128 })
export const MAX_QUEUED_REQUESTS = numberValue("MCP_MAX_QUEUED_REQUESTS", { max: 4096 })
export const MAX_SSE_STREAMS = numberValue("MCP_MAX_SSE_STREAMS", { max: 4096 })
