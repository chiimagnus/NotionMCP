...
const WINDOWS_TOKEN_PLACEHOLDER = "请替换为随机生成的64位十六进制字符串"
...
function windowsTokenValue() {
	const token = stringValue("MCP_TOKEN_WINDOWS")
	if (token === WINDOWS_TOKEN_PLACEHOLDER) throw new Error("请先替换 MCP_TOKEN_WINDOWS 的默认值")
	return token
}
...
function launcherConfig(platform) {
	...
	return {
		...
		...(platform === "macos"
			...
			: platform === "linux"
				? { tokenFile: configuredPath("MCP_LINUX_TOKEN_FILE") }
				: { token: windowsTokenValue() }),
	}
}
