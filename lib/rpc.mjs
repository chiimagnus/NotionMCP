...
// 同步写。日志最有价值的时刻恰恰是进程正要死的时候（fatal 退出、stdin EOF 退出），
// 而异步 appendFile 的 promise 在 process.exit() 之后就再也不会 resolve。之前那两条把
// 根因钉死在 EPIPE 上的 FATAL 记录，是靠 fatal() 里 setTimeout 50ms 侥幸赢了竞态才留
// 下来的 —— 审计日志不该赌运气。写日志本身失败则吞掉，绝不能反过来打死进程。
