...
import { MAX_ACTIVE_REQUESTS, MAX_QUEUED_REQUESTS } from "./config.mjs"
...
function requestMetadata(req, path, method) {
	...
	const transport =
		path === LEGACY_MESSAGES_PATH
			? "legacy_sse_message"
			: path === LEGACY_SSE_PATH || (path === "/mcp" && method === "GET")
				? "legacy_sse"
			...
			if (!accepts(req.headers.accept, "text/event-stream")) {
				...
				try {
					const session = await legacySse.open(res, (closed) =>
						trace("info", "sse_closed", { ...closed, legacySseSessions: legacySse.size, connections: sockets.size }),
					)
					trace("info", "sse_opened", { ...session, legacySseSessions: legacySse.size, connections: sockets.size })
				} catch (error) {
					trace("error", "failed", { error, status: 500 })
					throw error
				}
