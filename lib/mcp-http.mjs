...
function createSdkServer(httpSignal) {
	// ponytail: SDK 1.29.0 的 low-level Server 可直接复用现有 JSON Schema；
	// 高层 McpServer 会要求无收益地把四套 schema 重写成 Zod。
	...
	mcp.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
		...
		const signal = AbortSignal.any([extra.signal, httpSignal])
		try {
			return await handler(request.params.arguments || {}, { signal })
		} catch (error) {
			auditLog("tool", "failed", {
...
export function createMcpHttpServer({ port, token }) {
	...
	const handleRequest = async (req, res) => {
		...
		try {
			...
			mcp = createSdkServer(controller.signal)
			...
			if (controller.signal.aborted) {
				if (shuttingDown) sendAndClose(res, 503, "Service Unavailable")
				return
			}
			throw error
		} finally {
			try {
				...
				auditLog("http", "sdk_close_failed", { errorType: error?.name || "Error" })
			}
	...
	const httpServer = createServer((req, res) => {
		handleRequest(req, res).catch((error) => {
			auditLog("http", "request_failed", { errorType: error?.name || "Error" })
	...
	httpServer.on("error", (error) => {
		if (!shuttingDown) resolveUnexpected(error)
	})
	httpServer.on("close", () => {
		if (wasListening && !shuttingDown) resolveUnexpected(new Error("HTTP server closed unexpectedly"))
	})
