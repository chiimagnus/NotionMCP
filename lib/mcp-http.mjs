...
import { createProtocolHandler } from "./mcp-protocol.mjs"
...
export function createMcpHttpServer({ port, token }) {
	...
	const protocol = createProtocolHandler((error) => log("warning", "protocol", "rejected", { error }))
	...
	const handleRequest = async (req, res) => {
		...
		if (!isAllowedOrigin(req)) {
			logRejection(403, "origin_not_allowed")
		...
		try {
			...
			if (await protocol.isLegacy(webRequest, parsedBody)) {
				await protocol.serveLegacy(req, res, parsedBody, controller.signal)
			} else {
