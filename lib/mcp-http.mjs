...
import { MAX_ACTIVE_REQUESTS, MAX_QUEUED_REQUESTS, MAX_SSE_STREAMS } from "./config.mjs"
...
import { handlers } from "../tools/index.mjs"

const HOST = "127.0.0.1"
const MAX_BODY_BYTES = 1024 * 1024
const SSE_HEARTBEAT_MS = 25_000
const STREAMABLE_HTTP_VERSION = "2026-07-28"
const MCP_METHODS = new Set(["initialize", "tools/list", "tools/call", "notifications/initialized", "ping"])
...
function requestMetadata(req, path, method) {
	const protocol = singleHeader(req.headers["mcp-protocol-version"])
	const mcpProtocol = typeof protocol === "string" && /^\d{4}-\d{2}-\d{2}$/.test(protocol) ? protocol : undefined
	const rpcMethod = singleHeader(req.headers["mcp-method"])
	const tool = singleHeader(req.headers["mcp-name"])
	const transport =
		path === LEGACY_MESSAGES_PATH
			? "legacy_sse_message"
		: path === LEGACY_SSE_PATH
				...
				: path === "/mcp" && method === "POST"
					? mcpProtocol === STREAMABLE_HTTP_VERSION
						? "streamable_http"
						: "compat_http"
					: "other"
	return {
		...
		...(typeof rpcMethod === "string" && MCP_METHODS.has(rpcMethod) ? { mcpMethod: rpcMethod } : {}),
		...(typeof tool === "string" && Object.hasOwn(handlers, tool) ? { tool } : {}),
	}
}
...
function createStreamableSseManager() {
	...
	const closeSession = (session) => {
		...
		session.res.off("close", session.onResponseClose)
		session.res.off("error", session.onResponseError)
		session.onclose?.({ sseSession: session.label })
	}

	return {
		open(res, onclose) {
			const session = { closed: false, label: ++nextSession, onclose, res }
			session.onResponseClose = () => closeSession(session)
			session.onResponseError = () => closeSession(session)
			sessions.add(session)
			res.once("close", session.onResponseClose)
			res.once("error", session.onResponseError)
			try {
				res.writeHead(200, {
			...
			// ponytail: 25 秒注释帧低于常见代理空闲超时；若部署端有更短限制，再把它设为配置。
...
export function createMcpHttpServer({ port, token }) {
	...
	const legacySse = createLegacySseManager({
		onerror: (error, session) => log("warning", "legacySse", "failed", { error, sseSession: session?.label }),
	})
	const streamableSse = createStreamableSseManager()
	let sseHighWater = 0
	const sseCount = () => legacySse.size + streamableSse.size
	const sseStats = () => ({ sseStreams: sseCount(), maxSseStreams: MAX_SSE_STREAMS, sseHighWater })
	const recordSseHighWater = () => {
		sseHighWater = Math.max(sseHighWater, sseCount())
	}
	const healthSnapshot = createHealthSnapshot({
		version: SERVER_VERSION,
		maxActiveRequests: MAX_ACTIVE_REQUESTS,
		activeRequestCount: () => active.size,
		queuedRequestCount: () => queued.length,
		maxQueuedRequests: MAX_QUEUED_REQUESTS,
		connectionCount: () => sockets.size,
		legacySseCount: () => legacySse.size,
		streamableSseCount: () => streamableSse.size,
		maxSseStreams: MAX_SSE_STREAMS,
		sseHighWater: () => sseHighWater,
		isShuttingDown: () => shuttingDown,
	})

	const queueStats = () => ({ activeRequests: active.size, queuedRequests: queued.length, maxQueuedRequests: MAX_QUEUED_REQUESTS })
	const removeQueueAbort = (context) => {
		if (!context.onQueueAbort) return
		context.controller.signal.removeEventListener("abort", context.onQueueAbort)
		context.onQueueAbort = undefined
	}
	const removeQueued = (context) => {
		if (!context.queued) return false
		context.queued = false
		const index = queued.indexOf(context)
		if (index >= 0) queued.splice(index, 1)
		removeQueueAbort(context)
		return true
	}
	const startQueuedRequests = () => {
		while (!shuttingDown && active.size < MAX_ACTIVE_REQUESTS && queued.length > 0) {
			const context = queued.shift()
			if (!context.queued) continue
			context.queued = false
			removeQueueAbort(context)
			if (context.controller.signal.aborted) {
				context.rejectSlot(context.controller.signal.reason || new Error("HTTP request cancelled"))
				continue
			}
			active.add(context)
			context.resolveSlot()
	...
	const acquireRequestSlot = (context) => {
		if (shuttingDown) return Promise.reject(new Error("Server shutting down"))
		if (active.size < MAX_ACTIVE_REQUESTS && queued.length === 0) {
			active.add(context)
			return Promise.resolve()
		}
		if (queued.length >= MAX_QUEUED_REQUESTS) return Promise.reject(new QueueFullError())
		return new Promise((resolve, reject) => {
			context.queued = true
			context.resolveSlot = resolve
			context.rejectSlot = reject
			context.onQueueAbort = () => {
				if (!removeQueued(context)) return
				reject(context.controller.signal.reason || new Error("HTTP request cancelled"))
			}
			context.controller.signal.addEventListener("abort", context.onQueueAbort, { once: true })
			if (context.controller.signal.aborted) {
				context.onQueueAbort()
				return
			}
	...
	const finishRequestContext = (context) => {
		removeQueued(context)
	...
	const handleLegacyMessage = async (req, res, url, trace) => {
		...
		trace("info", "queued", { count: queued.length + 1, ...queueStats() })
		try {
			await acquireRequestSlot(context)
			trace("info", "started", queueStats())
			...
			try {
				...
				const session = await legacySse.handleMessage(req, res, sessionId, parsedBody)
				if (!session) {
					logRejection(404, "sse_session_not_found", trace)
					sendAndClose(res, 404, "Session Not Found")
					return
				}
				trace("info", "completed", { status: res.statusCode || 202, ...session })
		} catch (error) {
			if (error instanceof QueueFullError) {
				logRejection(429, "request_queue_full", trace)
				sendAndClose(res, 429, error.message, { "Retry-After": "1" })
				return
			}
	...
	const handleRequest = async (req, res) => {
		...
		const connectionId = registerSocket(req.socket)
		let metadata = {}
		const trace = (level, event, fields = {}) =>
			log(level, "http", event, { traceId, connectionId, elapsedMs: Date.now() - startedAt, ...metadata, ...fields })
		...
		const method = (req.method || "GET").toUpperCase()
		metadata = requestMetadata(req, path, method)
		...
		if (path === LEGACY_SSE_PATH) {
			...
			if (sseCount() >= MAX_SSE_STREAMS) {
				logRejection(503, "sse_capacity_reached", trace)
				sendAndClose(res, 503, "Service Unavailable", { "Retry-After": "1" })
				return
			}
			try {
				const session = await legacySse.open(res, (closed) =>
					trace("info", "sse_closed", { ...closed, legacySseSessions: legacySse.size, connections: sockets.size, ...sseStats() }),
				)
				recordSseHighWater()
				trace("info", "sse_opened", { ...session, legacySseSessions: legacySse.size, connections: sockets.size, ...sseStats() })
			} catch (error) {
				trace("error", "failed", { error, status: 500 })
				throw error
			}
		...
		if (path === "/mcp" && method === "GET") {
			...
			const session = streamableSse.open(res, (closed) =>
				trace("info", "sse_closed", { ...closed, streamableSseSessions: streamableSse.size, connections: sockets.size, ...sseStats() }),
			)
			recordSseHighWater()
			trace("info", "sse_opened", { ...session, streamableSseSessions: streamableSse.size, connections: sockets.size, ...sseStats() })
			return
		}
	...
	httpServer.on("connection", registerSocket)
	...
	const shutdown = (reason = "requested") => {
		...
		shutdownPromise = (async () => {
			const close = httpServer.listening
				...
				: Promise.resolve()
			const requests = [...pendingRequests].map((context) => context.done)
			for (const context of pendingRequests) context.controller.abort(new Error("Server shutting down"))
			await Promise.allSettled([legacySse.close(), streamableSse.close()])
	...
	return {
		...
		get legacySseCount() {
			return legacySse.size
		},
		get streamableSseCount() {
			return streamableSse.size
		},
	}
}
