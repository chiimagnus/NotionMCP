...
const MAX_LEGACY_SSE_SESSIONS = 32
const MAX_HTTP_CONNECTIONS = MAX_LEGACY_SSE_SESSIONS + MAX_ACTIVE_REQUESTS + 8
...
export function createMcpHttpServer({ port, token }) {
	...
	const legacySse = createLegacySseManager({
		maxSessions: MAX_LEGACY_SSE_SESSIONS,
		onerror: (error) => log("warning", "legacySse", "failed", { error }),
	})
	...
	const handleRequest = async (req, res) => {
		...
		if (path === HEALTH_PATH) {
			...
			const opened = await legacySse.open(res, () => trace("info", "sse_closed", { count: legacySse.size }))
			if (!opened) {
				logRejection(429, "too_many_sse_sessions", trace)
				sendAndClose(res, 429, "Too Many SSE Sessions")
				return
			}
			trace("info", opened.replaced ? "sse_replaced" : "sse_opened", { count: legacySse.size })
			return
		}
	...
	httpServer.maxConnections = MAX_HTTP_CONNECTIONS
