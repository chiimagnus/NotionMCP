...
const deliveredDigestByWorkDir = new Map()
...
export function formatAgentsMdContext(context, heading = "project context") {
...
export function markAgentsMdContextDelivered(context) {
	if (!context.cancelled) deliveredDigestByWorkDir.set(context.workDir, context.digest)
}

export async function getChangedAgentsMdBlock(workDir, options = {}) {
	const context = await getAgentsMdContext(workDir, options)
	if (context.cancelled) return ""
	const previous = deliveredDigestByWorkDir.get(context.workDir)
	markAgentsMdContextDelivered(context)
	if (previous === context.digest || context.sources.length === 0) return ""
	return formatAgentsMdContext(context, "auto-loaded dev conventions")
}
