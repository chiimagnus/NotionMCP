...
// 熔断后不再彻底放弃，而是转入低频重试。
const CIRCUIT_OPEN_RETRY_MS = 5 * 60_000
// 主动健康探测：间隔，以及连续几次无响应才判定为假死（避免瞬时抖动误杀）。
const HEALTH_PROBE_INTERVAL_MS = 60_000
const HEALTH_PROBE_FAILURES_BEFORE_RESTART = 2
...
class SupervisedService {
	...
	// ponytail: 只监听子进程的 exit 事件是不够的 —— “进程还活着但服务已不可用”
	// 真实发生过（机器睡眠唤醒后连接层失效、supergateway 会话卡死等），那时候监督器
	// 完全无感，用户侧表现就是“又连不上了，只能手动重启”。所以主动探一探。
	_armHealthProbe() {
		clearInterval(this.probeTimer)
		this.probeFailures = 0
		this.probeTimer = setInterval(async () => {
			if (this.stopped || shuttingDown || recycling || this.restarting) return
			if (!this.child || this.child.exitCode !== null) return
			if ((await requestStatus(this.healthUrl)) !== 0) {
				this.probeFailures = 0
				return
			}
			this.probeFailures += 1
			if (this.probeFailures < HEALTH_PROBE_FAILURES_BEFORE_RESTART) return
			logError(`\u26a0\ufe0f ${this.name} 进程还在，但连续 ${this.probeFailures} 次健康探测无响应，主动重启`)
			this.probeFailures = 0
			try {
				await this.restartForRecycle()
				logLine(`\u2705 ${this.name} 已通过健康探测自动恢复`)
			} catch (err) {
				logError(`\u274c ${this.name} 健康探测重启失败：${err.message || err}，将由崩溃重启逻辑接管`)
			}
	...
	// ponytail: 旧版在超过阈值时直接 requestStop() 退掉整条链路，用户只能手动重跑
	// up.ps1/up.sh。快速重启风暴确实该刹车，但“永久放弃”是另一回事 —— 故障往往是暂时的
	// （上游没起来、端口被占、机器刚睡醒），低频重试能自己走出来。
	// 同时把原来的递归重试改成循环，避免长期运行后累积 await 栈帧。
		...
		while (!this.stopped && !shuttingDown) {
			...
			if (this.restartCount > MAX_CONSECUTIVE_RESTARTS) {
				if (!this.circuitOpen) {
					...
					logError(
						`\u274c ${this.name} 在短时间内反复崩溃（超过 ${MAX_CONSECUTIVE_RESTARTS} 次），暂停快速重启，改为每 ${Math.round(CIRCUIT_OPEN_RETRY_MS / 60_000)} 分钟重试一次`,
					)
				}
				waitMs = CIRCUIT_OPEN_RETRY_MS
			} else {
				waitMs = Math.min(RESTART_BACKOFF_MAX_MS, RESTART_BACKOFF_BASE_MS * 2 ** (this.restartCount - 1))
				logLine(`${this.name} 将在 ${waitMs}ms 后自动重启（第 ${this.restartCount} 次）`)
			}
			await sleep(waitMs)
