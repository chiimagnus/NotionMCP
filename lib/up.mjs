...
async function main() {
	...
	const logModule = await import("./audit-log.mjs")
	auditLog = logModule.auditLog
	...
	auditLog("launcher", "http_started")
	...
	funnelOwned = true
	auditLog("launcher", "funnel_started")
...
try {
	...
	auditLog?.("launcher", "failed", { errorType: error?.name || "Error" })
	process.exitCode = 1
} finally {
	await cleanup()
}
