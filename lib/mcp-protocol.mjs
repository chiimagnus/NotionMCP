...
const SERVER_INSTRUCTIONS = `这个 MCP 提供 6 个工具：read_rules（规则与 skills 清单）、load_skills（技能正文）、read_file（文本）、read_image（图片）、run_command（命令）和 apply_patch（编辑）。

- 这不是严格沙盒：cwd/path 只是默认工作目录，绝对路径、cd 切换等仍可访问 ${SANDBOX_DIR} 之外的文件。
- 开发任务先调用 read_rules；它会返回所有 skills 的 key、name、description，按需再用 load_skills 读取一个正文；先用 read_file/read_image 理解现状，最后才执行命令或写入。
...
// ponytail: Notion 仍在使用 2024 HTTP+SSE；仅该 transport 依赖 v1 SDK，迁移完成后删除。
export function createLegacySseServer(signal) {
	let activeTools = 0
	...
	mcp.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
		...
		if (activeTools >= 1) {
			return { content: [{ type: "text", text: "Error: Too many active tool calls on this SSE session" }], isError: true }
		}
		activeTools += 1
		try {
			return await runTool(request.params.name, request.params.arguments, AbortSignal.any([signal, extra.signal]))
		} finally {
			activeTools -= 1
		}
	})
...
export function createProtocolHandler(onerror) {
	const modern = createMcpHandler(createModernServer, { onerror })
	return {
		close: () => modern.close(),
