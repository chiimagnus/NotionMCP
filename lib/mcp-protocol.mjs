...
const SERVER_INSTRUCTIONS = `这个 MCP 提供 5 个工具：read_rules（规则与 skills 清单）、read_file（文本）、read_image（图片）、run_command（命令）和 apply_patch（编辑）。

- 这不是严格沙盒：cwd/path 只是默认工作目录，绝对路径、cd 切换等仍可访问 ${SANDBOX_DIR} 之外的文件。
- 开发任务先调用 read_rules；它会返回所有 skills 的 key、SKILL.md 路径、name、description，按需再用 read_file 读取技能正文；先用 read_file/read_image 理解现状，最后才执行命令或写入。
...
- skills 目录或 SKILL.md 的 mtime 变化会刷新清单。`
