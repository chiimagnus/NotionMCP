...
async function main() {
	...
	const execServer = join(MCP_ROOT, "exec-server.mjs")
	...
	const proxy = startChild("auth-proxy", process.execPath, [join(MCP_ROOT, "auth-proxy.mjs")])
	...
	const status = runTailscale(["funnel", "status", "--json"])
