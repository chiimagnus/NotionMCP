...
export const definition = {
  ...
  title: "Read Rules",
  description:
    "只读地加载全局规则文件、当前项目从外到内的 AGENTS.md/CLAUDE.md，以及所有可用 skills 的 SKILL.md 路径、name、description。开始开发任务时先调用，随后用 read_file 按需读取技能正文。",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
        type: "string",
        description: "可选，相对默认工作目录的子目录或绝对目录。",
      },
    },
  },
};
...
function skillLocations(dir, relPath = "", signature = []) {
  ...
  try {
    ...
    if (error?.code !== "ENOENT")
      log("warning", "read_rules", "catalog_scan_failed", {
  ...
  try {
    ...
    if (skillInfo.isFile()) {
      signature.push(
        `${relPath}/SKILL.md:${skillInfo.mtimeMs}:${skillInfo.size}`,
      );
      return [{ dir, file }];
    }
  } catch (error) {
  ...
  const entries = locations
    .map((location) => {
      ...
      try {
        ...
        log("warning", "read_rules", "frontmatter_read_failed", {
          errorType: error?.name || "Error",
        });
      }
      return {
...
export async function call(args, { signal } = {}) {
  ...
  if (cwd !== undefined && typeof cwd !== "string") {
    return {
      content: [{ type: "text", text: "Error: cwd must be a string" }],
      isError: true,
    };
  }
  ...
  if (context.cancelled)
    return {
      content: [
        { type: "text", text: "Error: Project context read cancelled" },
      ],
      isError: true,
    };
  if (signal?.aborted)
